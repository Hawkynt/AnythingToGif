...
| `release.yml`                   | tag push `v*` + manual dispatch     | Cut a GitHub Release                    |
| `nightly.yml`                   | successful CI run on `master`/`main`| Publish `nightly-YYYY-MM-DD` prerelease |
| `_build.yml`                    | `workflow_call` (internal)          | Shared CLI-zip + UI-zip block           |
...
- Every `v*` tag cuts a proper release carrying the CLI and UI zips.
- Old nightlies are auto-pruned on a **Grandfather-Father-Son** schedule — 7 daily + 4 weekly + 3 monthly.

## Why it's built this way

- **No cron triggers.** The old `Build.yml` ran weekly and pushed NuGet packages from every scheduled build — silently shipping from unreviewed code. This pipeline only fires on actual events.
...
- **Nightly CLI only.** A nightly carries the CLI zip; the UI zip is cut with a real release.
